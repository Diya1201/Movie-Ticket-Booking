# Check the login details text file for login credentials and how to run the system
import sys
...
def run_query(query):
    try:
        ...
        if db.is_connected():
            # Print connection status and query information
...
# Flask route for rendering login page
@app.route('/')
def render_login_page():
    return render_template('login.html')

# Flask route for login verification
@app.route('/login', methods=['POST'])
def verify_and_render_respective():
    username = request.form['username']
    password = request.form['password']

    try:
        if username == 'cashier' and password == 'cashier123':
            res = run_query('call delete_old()')
